...
public class Folder {
	...
	// Create the folder and mouse entity
	public static Entity generate(float fX, float fY, float fZ , float fR){
		
		if (staticModel == null){ // We load the file only once 
			model = OBJLoader.loadOBJModel("folder");
			texture = new ModelTexture(GameVars.loader.loadTexture("folder"));
			staticModel = new TexturedModel(model, texture);
		}
		return new Entity(staticModel, new Vector3f(fX,fY,fZ),0,fR,0,0.4f, "Folder", true);
	}
}
